...
BACKTRACKING_MODE=${BACKTRACKING_MODE:-submit}
echo "Using BACKTRACKING_MODE=$BACKTRACKING_MODE"
...
for dataset in "${datasets[@]}"; do
    ...
    for file in outputs/traces/$dataset/traces_*.pkl; do
        ...
        mode_args=()
        if [[ "$BACKTRACKING_MODE" == "check" ]]; then
            mode_args+=(--check-batches)
        fi

        srun --exclusive -n1 bash -c "PYTHONUNBUFFERED=1 python3 -u -m src.scripts.analyze_backtracking --dataset '$dataset' --model '$model' --output-dir outputs/backtracking_analysis ${mode_args[*]}" &
        running=$((running+1))
